...
#Merging Data and making to individual level
...
#Only Urban
Labor_Force <- Labor_Force %>%
  filter(Labor_Force$Sector %in% c("2"))
...
#Removing Weights NSS, NSC,MLT, Weights
Labor_Force<-Labor_Force%>%select(Usual_Principal_Activity_Status,General_Education,Sex,Age,Religion,Social_Group,Land_Owned,Marital_Status, State)
...
#NOT CREATING THIS BECAUSE Filted out Only URBAN AREA 
...
str(Labor_Force)
colnames(Labor_Force)<-c("Female_Labor_Force_Participation","General_Education","Sex","Age","Religion","Social_Group","Land_Owned","Marital_Status", "State" )
logistic_reg_Urban <- glm(Labor_Force$Female_Labor_Force_Participation ~ Labor_Force$General_Education + Labor_Force$Age  + Labor_Force$Religion + Labor_Force$Social_Group + Labor_Force$Land_Owned + Labor_Force$Marital_Status,
                          data = Labor_Force,  family = "binomial" (link="logit"))

stargazer(logistic_reg_Urban,type="text",out="models.txt")


#PROBIT MODEL
probit_reg_Urban <- glm(Labor_Force$Female_Labor_Force_Participation ~ Labor_Force$General_Education + Labor_Force$Age + Labor_Force$Religion + Labor_Force$Social_Group + Labor_Force$Land_Owned + Labor_Force$Marital_Status,
                        data = Labor_Force,  family = "binomial" (link="probit"))

#C-LOG-LOG MODEL
cloglog_reg_Urban <- glm(Female_Labor_Force_Participation ~ General_Education + Age + Age^2 + 
                           Religion + Social_Group + Land_Owned + Marital_Status,
                         data = Labor_Force, family = binomial(link = "cloglog"))

stargazer(probit_reg_Urban,type="text",out="models.txt")
options(scipen=999)
summary(logistic_reg_Urban)
summary(probit_reg_Urban)
cat("\nCloglog Regression - Urban Sector\n")
summary(cloglog_reg_Urban)



#To Plot
xtabs(~Labor_Force$General_Education, data = Labor_Force)
count <- c(96190, 48289, 54656 ,45411, 34466,  2651, 28478 , 9657)
barplot(count, names=c("illiterate","Primary", "middle", "secondary","higher secondary","diploma", "graduate","post graduate"),
        main="General Education in Urban Area ", xlab = "Education level",ylab="Number of people", 
        col=c("orange","pink","blue","grey","green","red","cyan","yellow"), cex.names=0.75, las = 2 ,
        cex.main = 1,cex.axis = .7, border ="red", density = 145)
...
pie(count, labels = labels_pct, 
    main = "General Education in Urban Area (in %)", 
...
barplot(count, names=c("Male","Female"),
        ...
        col=c("orange","blue"), cex.names=0.75, las = 1 ,
        cex.main = 1,cex.axis = .7, border ="red", density = 145)


##REligion
xtabs(~Labor_Force$Religion, data = Labor_Force)
count <- c(216288 ,68746,  21061,   7379,   1857,   2060,  24 ,  2383)
barplot(count, names=c("Hinduism","Islam","Christanity","Sikhism","Jainism","Buddhism","Zoroastrianism","others"),
        main="Religion in Urban Area ", xlab = "Religion",ylab="Number of people", 
        col=c("orange","pink","blue","grey","green","red","cyan","yellow"), cex.names=0.75, las = 2 ,
        cex.main = 1,cex.axis = .7, border ="red", density = 145)
...
count <- c(125549 , 29585,  41237, 123427)
barplot(count, names=c("Others","ST","SC","OBC"),
        main="Social Group in Urban Area ", xlab = "Social Group",ylab="Number of people", 
        col=c("orange","blue","grey","green "), cex.names=0.75, las = 1 ,
        cex.axis = .7, border ="red", density = 145)


##Marital Status
xtabs(~Labor_Force$Marital_Status, data = Labor_Force)
count <- c(124048,169753 , 24313 ,  1684)
barplot(count, names=c("Never Married","Currently Married","Widowed","Divorced"),
        main="Marital Status in Urban Area ", xlab = "Marital Status",ylab="Number of people", 
        col=c("orange","pink","blue","grey","green"), cex.names=0.75, las = 1 ,
        cex.axis = .7,cex.lab = 0.90, border ="red", density = 145)
